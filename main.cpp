...
void loadTexture(const char* file_name, int tex_id)
{
	Image* image1;
...
void moveHelicopter(void) {
	...
	curX = (float)50 * ((cos(ro * PI / 180) / (1 + sin(ro * PI / 180) * sin(ro * PI / 180))));
	curZ = (float)50 * ((cos(ro * PI / 180) * (sin(ro * PI / 180))) / (1 + sin(ro * PI / 180) * sin(ro * PI / 180)));
...
void draw_open_box(GLfloat size)
{
	float x1, y1, z1;
	float x2, y2, z2;
	float x3, y3, z3;
	...
	glPushMatrix();
	{
		...
		glBegin(GL_QUADS);
		{
			...
			glTexCoord2f(1, 0);
			glVertex3f((x8 + x7) / 2, y7, z7);

			glTexCoord2f(1, 1);
			glVertex3f((x6 + x5) / 2, y6, z6);
...
void handleResize(int w, int h)
{
	...
	glLoadIdentity();
	gluPerspective(60, (GLfloat)w / h, 5, 100);
...
int main(int argc, char** argv)
{
    ...
    //loadTexture("wood.bmp", 1);     //wall
	//loadTexture("cont.bmp", 2);  // ceil

    loadTexture("floor.bmp", 3);      //floor
	loadTexture("wood.bmp", 4);   //table
	loadTexture("wood.bmp", 5);    //shelf && chair
	loadTexture("mike.bmp", 6);   // box
	loadTexture("cotton.bmp", 7);     // bed
